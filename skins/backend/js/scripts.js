...
var modalLoading = (function ($) {
    ...
    var $dialog = $(
            ...
            '<div class="text-center" data-backdrop="static">' +
            '<img width="50" height="50" src="skins/backend/images/loading_5.gif">' +
            '<h4>Đang xử lý... <button type="button" class="close" style="float: none;" data-dismiss="modal" aria-hidden="true">x</button></h4>' +
            '</div>' +
            '</div>' +
            '</div>' +
            '</div>'
...
var noticeMeseage = (function ($) {
    return {
        clear: function () {
            $('.notice .alert').removeClass('alert-danger alert-warning alert-success');
        },
        show: function (messeage, status) {
            this.clear();
            $('.alert-message').html(messeage);
            $('.notice .alert').addClass('alert-' + status);
            $('.notice').show();
        },
        hide: function () {
            $('.notice').hide();
