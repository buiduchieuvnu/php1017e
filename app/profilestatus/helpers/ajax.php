...
define('AREA','A');
require '../../../init.php';
require_once ('../profilestatus.php');
...
switch ($act) {
	case 'getclass':
		...
		$list_class = ProfileStatusApp::getListClass($id_course, $id_group_field);
		echo json_encode($list_class);
		break;
	case 'getstudent':
		$id_class = (isset($_POST['id_class'])) ? intval($_POST['id_class']) : 0;
		$list_students = ProfileStatusApp::getStudenByClass($id_class);
	...
	case 'update':
		...
		if (!empty($postData)) {
			foreach ($postData as $data) {
				list($id_profile, $status) = explode(';', $data);
				if(!ProfileStatusApp::updateStatus($id_profile, $status))
					$flag = false;
			}
			if ($flag) {
				$status = 'success';
				...
				$status = 'danger';
				$message = 'Error: Đã có lỗi trong quá trình cập nhật dữ liệu';
			}
		}
		else {
			$status = 'danger';
			$message = 'Error: Không tồn tại dữ liệu. Vui lòng thử lại';
		}
